...
export class OllamaRuntimeView extends ItemView {
    ...
    this.renderCollapsibleCard(
      ...
      (body) => {
        const refresh_button = body.createEl("button", {
          text: this.context_tables_loading
            ? "Refreshing..."
            : "Refresh Tables",
          cls: "mod-cta",
        });
  ...
  private renderCollapsibleCard(
    container: HTMLElement,
    key: keyof typeof this.section_open,
    title: string,
    subtitle: string,
    render_body: (body: HTMLElement) => void,
    prefix?: HTMLElement,
  ): void {
  ...
  private renderStatusDot(): HTMLElement {
    const dot = document.createElement("span");
    dot.className = `ollama-shell__status-dot ollama-shell__status-dot--${
      this.status_tone === "ok" ? "ok" : "error"
    }`;
    return dot;
  }
  ...
  private renderMetaRow(
    ...
    const row = container.createDiv({
      cls: `ollama-shell__status ollama-shell__status--${tone}${
        copy_on_click ? " ollama-shell__status--copyable" : ""
      }`,
    });
    row.createSpan({
    ...
    row.createDiv({
      ...
      attr: copy_on_click ? { title: value } : undefined,
    });
    if (copy_on_click) {
      row.addEventListener("click", async () => {
        try {
          ...
          new Notice(`Could not copy ${label.toLowerCase()}.`);
        }
      });
    }
  }

  private truncateCardValue(value: string, max_length = 68): string {
